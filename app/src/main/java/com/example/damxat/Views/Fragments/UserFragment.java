...
public class UserFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // Here we calling the get users function
        getUsers();
    ...
    public void getUsers(){
        // Here we are getting a user from firebase and getting the reference to it
        firebaseUser = FirebaseAuth.getInstance().getCurrentUser();
        ref = FirebaseDatabase.getInstance().getReference("Users");

        ArrayList<User> arrayUsers = new ArrayList<>();

        // We add a listener to the reference
        ref.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                arrayUsers.clear();
                // Here we are going by every child that the Users collection and adding it to the user model
                // If the user's id isn't equal to the firebase user uid then we add the user to our user list
